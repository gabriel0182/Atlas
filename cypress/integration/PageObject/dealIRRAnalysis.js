...
class dealIRRAnalysis {
  ...
  selectIRR() {
    ...
    const IRROption = cy.contains("IRR Analysis");
    IRROption.click({ force: true });
    IRROption.wait(2000);
    const choosetranche = cy
      .get(
        ":nth-child(1) > .nativ-form-group > .css-1pcexqc-container > .css-1ofrjoz-control > .css-1hwfws3"
      )
      .click({ force: true });
    ...
    const chooseForecast = cy
      .get(
        ":nth-child(2) > .nativ-form-group > .css-1pcexqc-container > .css-1ofrjoz-control > .css-1hwfws3"
      )
      .click({ force: true });
    chooseForecast.type(`${this.forecast}{enter}{enter}`);
    const run = cy.get(":nth-child(3) > .nativ-button");
    run.click({ force: true });
    run.wait(2000);
    const verifyTable = cy
      .get('[class="col"]')
  ...
  confirmValues() {
    const IRRValue = cy.get(
      ":nth-child(2) > .container > :nth-child(1) > :nth-child(2)"
    );
    IRRValue.should("not.equal", 0);
    IRRValue.should("not.equal", "-1.20%");
    IRRValue.should("not.equal", "(1.20%)");
    const proFit = cy.get(
      ":nth-child(2) > .container > :nth-child(2) > :nth-child(2)"
    );
    proFit.should("not.equal", 0);
    proFit.should("not.equal", "($1,598,093)");
    proFit.should("not.equal", "$-1,598,093");
    const mulTiple = cy.get(
      ":nth-child(2) > .container > :nth-child(3) > :nth-child(2)"
    );
